...
whileAction calc action = do
  n <- calc
  when n loop
  where
    loop = do
      action
      whileAction calc action
...
boolCalc (Reln cOp expr1 expr2) = do
  n1 <- (intCalc expr1)
  n2 <- (intCalc expr2)
  let relnFunc = (opLookup cOp relnOpTable)
  return $ relnFunc n1 n2
boolCalc (BBin op expr1 expr2) =  do
  n1 <- (boolCalc expr1)
  n2 <- (boolCalc expr2)
  let bBinFunc = (opLookup op bBinOpTable)
  return $ bBinFunc n1 n2
...
intCalc (ABin op expr1 expr2) = do
  n1 <- (intCalc expr1)
  n2 <- (intCalc expr2)
  let aBinFunc = (opLookup op aBinOpTable)
  return $ aBinFunc n1 n2
